...
Generating with Wan v2.7 (and the Kling / Aurora fal.ai models) could fail with a
bare `422 ValidationError` and the unhelpful console message
`fal.ai video generation failed`. The fal SDK puts the actual cause in
`error.body.detail`, but the catch blocks only read `error.message` (which is just
`"ValidationError"`), so the real reason was thrown away.

- `services/falService.ts`: added a shared `parseFalError()` helper that extracts
  `error.body.detail` (handling both the string form and the `[{ loc, msg }]`
  array form) and rewrote the Wan, Kling V2V, Kling v2.6 Motion Control, and
  Aurora catch blocks to use it. Moderation/NSFW rejections are now reported
  explicitly, including a note that disabling the safety checker only turns off
  the optional moderation layer — fal.ai still enforces a platform-level content
  policy that **cannot** be bypassed by `enable_safety_checker: false`.

Note on the "safety check bypass": the bypass itself is wired correctly —
`enableSafetyChecker` flows through to `enable_safety_checker: false`, which the
live Wan v2.7 schema accepts. The 422 was not caused by that flag; it is fal.ai's
platform content policy rejecting the request, now surfaced clearly.

## [1.5.0] — 2026-06-15

### Added — GPT Image 2 (OpenAI) as a selectable image-generation engine

Storyboard frames, alters, and character/location assets can now be generated by
OpenAI's **GPT Image 2** model as an alternative to the default Gemini ("Nano
Banana") engine. The choice is a global, per-browser setting that applies across
all projects.

- `services/openaiImageService.ts` (new): wraps the OpenAI Images API
  (`gpt-image-2`) for text-to-image generation, image alteration with reference
  inputs, and asset generation. Reads the key from `localStorage` (`openai_api_key`).
...
- `components/ProjectEditor.tsx` & `components/agent/AIAgent.tsx`: image-generation
  ...
  Gemini functions directly.
- `components/ProjectEditor.tsx`: added an **OpenAI API Key** input and an **Image
  Generation Engine** toggle (Gemini vs. GPT Image 2) in Project Settings; the
  OpenAI key is included in the "Save All API Keys" action.

> Note: Video generation, upscaling, and chat-edit continue to use Gemini
> regardless of the selected image engine.
