...
export const generateShotImage = async (
  ...
  // 🔒 STRICT LIKENESS HARD-LOCK: when on AND we have a portrait, we strip all
  // free-text physical/wardrobe/age description for that character. Reason: if
  // the description was auto-generated by an earlier broken render (e.g.
  // "weathered older man with beard"), Gemini trusts the text over the image
  // and the bad description re-creates the bad face on every regen — even with
  // the locked portrait staring at it. In hard-lock mode the portrait IS the
  // appearance source of truth, full stop.
  let textContext = "";
...
CRITICAL: This must look like a DIFFERENT CAMERA ANGLE of the SAME SCENE - not a new image. The person must be recognizably THE SAME PERSON from the input image.`;
      ...
      // 🔒 STRICT LIKENESS HARD-LOCK — route through "EDIT THE PORTRAIT" mode.
      // This is the same trick that already works in the reference-shot path:
      // when an image is sent FIRST with explicit edit-style framing, Gemini
      // anchors identity to *that* image far more reliably than when the
      // image is later in the parts array as a `REFERENCE_*`. Previously we
      // were front-loading the portrait but framing it as a "lock reference"
      // — Gemini still treated it as one of N references and got out-voted
      // by the Skybox figure / location picture / its own preconceptions.
      // Now in hard-lock mode, the FIRST character portrait becomes THE
      // image we are editing, with an explicit "preserve this exact face"
      ...
      if (lockPrimaryChar) {
        // Slot 0: the primary character portrait is the EDIT BASE.
        parts.push({
          inlineData: {
            mimeType: getMimeType(lockPrimaryChar.imageUrl!),
            data: stripBase64Header(lockPrimaryChar.imageUrl!)
          }
        });
        parts.push({
          text: `🔒 EDIT THIS PORTRAIT — STRICT LIKENESS HARD-LOCK
This is "${lockPrimaryChar.name}". The face in this portrait IS the character.
You are NOT creating a new character. You are taking THIS exact person and
placing them into a new shot.

PRESERVE EXACTLY (do NOT change):
- Face, bone structure, jawline, cheekbones, nose
- Eyes (color, shape, spacing), eyebrows
- Hairline, hair color, hair style (only adjust if explicitly required by the shot)
- Skin tone, skin texture, skin marks/freckles
- Age — DO NOT age, de-age, or otherwise alter the perceived age of this person
- Beard / no-beard state — match this portrait. If the portrait is clean-shaven,
  the rendered character must be clean-shaven. If bearded, must be bearded.

You MAY change:
- Pose, body position, gesture, expression
- Clothing / wardrobe (to fit the shot's wardrobe context)
- Framing (close-up, wide, etc.)
- Lighting and color grade (to match the location)
- Background / environment (will be the location below)

Any other reference image that follows — director ref photos, location image,
turnarounds, adjacent shots — MUST yield to THIS portrait on questions of
who this person is. They contribute style, mood, environment, composition only.`
        });
        // Selected turnarounds for the lock character — still useful as
        // multi-angle facial reference.
        const lockTurnarounds = (lockPrimaryChar.turnaroundImages || []).filter(t => t.isSelected);
        lockTurnarounds.forEach((t, tIdx) => {
          parts.push({
            inlineData: {
              mimeType: getMimeType(t.imageUrl),
              data: stripBase64Header(t.imageUrl)
          ...
          parts.push({
            text: `🔒 STRICT_LIKENESS_TURNAROUND_${tIdx + 1} for "${lockPrimaryChar.name}" (${t.angle}) — additional facial-identity reference for the SAME person above.`
          });
        });
        // Other characters in the shot (if any) still get the front-load
        // STRICT_LIKENESS_LOCK label so multi-character shots work.
        activeCharacters
          .filter(c => c.id !== lockPrimaryChar.id && !!c.imageUrl)
          .slice(0, 4)
          .forEach(char => {
            parts.push({
              inlineData: {
            ...
            parts.push({
              text: `🔒 STRICT_LIKENESS_LOCK_${char.name}: This is the OTHER character "${char.name}" who appears in this shot. Same NON-NEGOTIABLE identity rules as the edit base above.`
            });
            const selectedTurnarounds = (char.turnaroundImages || []).filter(t => t.isSelected);
            selectedTurnarounds.forEach((t, tIdx) => {
              parts.push({
              ...
              parts.push({
                text: `🔒 STRICT_LIKENESS_TURNAROUND_${tIdx + 1} for "${char.name}" (${t.angle}) — additional facial-identity reference.`
              });
            });
          });
      } else if (strictLikeness && hasActiveCharPortrait) {
        // Compose-mode fallback — keep old front-load behaviour because the
        // compose path needs scene + character refs in their own specific
        // order.
...
Adjacent shot continuity is SECONDARY to matching this reference.`
      ...
      if (activeLocation && activeLocation.imageUrl) {
        ...
        // 🔒 In strict mode, explicitly tell the model to ignore people in the
        // location image. Auto-generated location images often hallucinate a
        // figure/silhouette in the frame (e.g. a person standing on a rooftop
        // observation deck), and Gemini will happily transfer that face to
        // our character if we don't shut it down.
        const locationPeopleClause = (strictLikeness && hasActiveCharPortrait)
          ? `
⚠️ PEOPLE BLOCK: If this location image contains ANY people, faces, silhouettes,
or figures, treat them as nonexistent. NO facial features, body type, age, or
clothing from any person in this location image may transfer to the character.
Use this image ONLY for: architecture, furniture, walls, lighting, color grade,
atmosphere, time-of-day, weather, and props.`
          : "";
        parts.push({
...
Do NOT create a different room - use THIS room.${locationPeopleClause}`
        });
